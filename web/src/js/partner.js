            ...
            <div className="img-text">
                <div className="item">
                    <div className="img-box">
                        <img src="./assets/img/vip-pic1.jpg" />
                    </div>
                ...
                <div className="item">
                    <div className="text-box">
                        <img src="./assets/img/vip-pic2.jpg" />
                        <div className="info">
                ...
                <div className="item">
                    <div className="img-box">
                        <img src="./assets/img/vip-pic3.jpg" />
                    </div>
                    <div className="text-box">
                        <img src="./assets/img/vip-pic3.jpg" />
                        <div className="info">
...
var PartnerJoin = React.createClass({
    ...
    handleClickSubmit: function () {
        var $join = $(this.refs.join.getDOMNode());
        var formdata = {};
        var postFlag = true;
        var that = this;
        $join.find("input[name=firstname]");
        formdata.firstname = $join.find("input[name=firstname]").val();
        formdata.lastname = $join.find("input[name=lastname]").val();
        formdata.company = $join.find("input[name=company]").val();
        formdata.country = $join.find("select[name=country]").val();
        formdata.email = $join.find("input[name=email]").val();
        formdata.msg = $join.find("textarea[name=msg]").val();

        if(formdata.firstname == "" ) {
            $join.find("input[name=firstname]").next().show();
            postFlag = false;
        };
        if(formdata.lastname == "" ) {
            console.info("This field is required");
            $join.find("input[name=lastname]").next().show();
            postFlag = false;
        };
        if(formdata.country == "" ) {
            console.info("This field is required");
            $join.find("select[name=country]").next().show();
            postFlag = false;
        };
        if(formdata.email == "" || ( formdata.email != "" && !/.+@.+\.[a-zA-Z]{2,4}$/.test(formdata.email))){
            console.info("It seems to be invalid Email");
            $join.find("input[name=email]").next().show();
            postFlag = false;
        };

        $join.find("input,select").focus(function(){
            $(this).next().hide();
            postFlag = true;
        });
...
You can join our Global Partner Group on Facebook." />});
                } else if(data.success == 400) {
                    that.setState({tip: <Tips tipType="errow" clickCallback={that.handleClickCloseCb} tipTitle="Sorry!" tipCnt="Lack of email addresses or already exists" />});
                } else {
                    that.setState({tip: <Tips tipType="errow" clickCallback={that.handleClickCloseCb} tipTitle="Sorry!" tipCnt="Server exception!" />});
                }
                        ...
                        <form action="">
                            <div className="form-box-l">
                                ...
                                <div className="dd">
                                    <ul>
                                        <li className="fore1">
                                            <div className="ipt f-fl">
                                                <s className="ico-people"></s><input type="text" required placeholder="Firsrt Name" name="firstname"/>
                                                <div className="err-msg"><i></i><span>This field is required</span></div>
                                            </div>
                                             <div className="ipt f-fr">
                                                 <s classNameico-people></s>
        ...
        return (
            <div ref="tip">
                ...
                <div className={this.props.tipType == "success" ? "tips success" : "tips errow"}>
                    ...
                    {this.props.tipType == "success" && <a href="https://www.facebook.com/Wisape-Story-Builder-968758586520540/"  onClick={this.props.clickCallback}  className="btn-facebook"></a>}
                </div>
            </div>
